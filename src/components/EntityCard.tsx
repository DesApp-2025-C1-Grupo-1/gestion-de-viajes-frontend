...
export default function EntityCard({ title, subtitle, icon, fields, onView, onEdit, onDelete ,headerAction}: EntityCardProps) {
    ...
    return (
        <Card className="w-full border border-line rounded-lg overflow-hidden">
            <header className="p-5 bg-gray-100 flex justify-between items-center gap-4 border-b border-line">
                <div className="flex gap-4">
                    <div className="p-3 rounded-full shadow-sm bg-primary-orange text-white h-max w-max">
                        {icon}
                    </div>
                    <div className="flex flex-col justify-center">
                        <Typography variant="h6" fontWeight="bold" sx={{ fontSize: '1.10rem' }} className="text-gray-800">
                            {title}
                        </Typography>
                ...
                {headerAction && (
                    <Button
                        ...
                        sx={{ textTransform: 'none' , borderRadius: '999px', fontSize: '0.8rem', fontWeight: 600, color: headerAction.color || 'text.primary', borderColor: headerAction.color || '#B9B9B9', backgroundColor: "white", "&:hover": { borderColor: headerAction.color || 'grey.700', backgroundColor: "white" } }}
                    >
                        {headerAction.label}
                    </Button>
                )}
            </header>
            <article className="p-5 grid gap-4 sm:grid-cols-2">
